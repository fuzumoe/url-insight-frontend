...
// Mock Typography so that it renders an element with a data-testid based on the variant and color.
vi.mock('../Typography', () => ({
  default: vi.fn(
    ({
      children,
...
describe('TextInput', () => {
  ...
  it('displays error message when error prop is provided', () => {
    ...
    expect(errorMessage.closest('div')).toHaveAttribute(
      'id',
      `${defaultProps.id}-error`
    );
  });
  ...
  it('displays help text when provided and no error exists', () => {
    ...
    expect(helpText.closest('div')).toHaveAttribute(
      'id',
      `${defaultProps.id}-help`
    );
  });
